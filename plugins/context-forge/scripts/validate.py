...
single governance set and config. Python 3 stdlib only (no dependencies, so it runs in CI as-is). Checks:
  ...
  4. no absolute host path leaks into the plugin (portability guard);
  5. the reference files shared across the two skills (principles.md, config-protocol.md) are
...
Exit 0 when clean; exit 1 with one MISSING:/LEAK:/DRIFT:/YAML:/ERROR: line per finding.
"""
...
# machine it was authored on. Generic illustrative paths (services/foo, apps/web, src/...) are
...
def self_test():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # Frontmatter missing 'description'; dangling internal reference; absolute host-path leakage:
